...
6. Improvements: confidence-based ranking and better duplicate handling.`
  ...
  if (includesAny(q, ['voiceguru', 'voice guru', 'tutoring', 'voice tutor', 'multilingual'])) {
    return projectAnswer(
      'VoiceGuru: complexity and research summary',
      `1. Problem framing: create accessible, personalized learning for students across different proficiency levels using voice interaction in multiple Indian languages.
2. Technical constraints: accurate multilingual speech recognition, offline functionality with SQLite caching, real-time adaptive difficulty classification, WhatsApp integration for parent engagement.
3. Research process: evaluated voice processing pipelines, difficulty classification models, and offline caching strategies for low-bandwidth scenarios.
4. Implementation: multilingual voice processing engine with Kannada/Hindi/Tamil/English support, adaptive difficulty classifier (Beginner/Intermediate/Advanced), SQLite offline library, Twilio WhatsApp parent reporting, Vision Solver for homework images, and gamification system.
5. Validation: tested curriculum alignment for Karnataka State Board and CBSE, verified multilingual accuracy across regions, checked offline functionality with various caching strategies, and validated gamification engagement.
6. Improvements: enhanced Vision Solver accuracy, expanded language support, better adaptive tuning based on student interaction patterns, and improved parent reporting granularity.`
    );
  }

  if (includesAny(q, ['promptgate', 'prompt gate', 'algorand', 'escrow', 'micropayments', 'on-chain', 'algo'])) {
    return projectAnswer(
      'PromptGate: complexity and research summary',
      `1. Problem framing: build a secure AI prompt gateway that only returns model responses after Algorand TestNet micropayment escrow is settled.
2. Technical constraints: on-chain escrow security, payment verification, model selection cost trade-offs, and smooth user experience across blockchain and AI flows.
3. Research process: evaluated Algorand SDK flows, prompt complexity strategies, and publisher reputation/pricing models.
4. Implementation: FastAPI backend with Algorand escrow lock/release/payout, React frontend, Ollama LLM inference, complexity-based model selection, publisher registration, and QR-based TestNet account verification.
5. Validation: tested escrow lifecycle, payment challenge endpoints, pricing/reputation updates, and prompt pipeline reliability.
6. Improvements: tighter cost transparency, deeper prompt analytics, and more robust publisher reputation safeguards.`
    );
  }

  if (asksComplexity || asksResearch) {
    return `Chethan's projects generally follow this depth-oriented engineering pattern:\n\n1. Problem framing: define target impact and hard constraints first.\n2. Research: compare multiple candidate approaches and trade-offs.\n3. Architecture: choose components based on reliability, real-time needs, and iteration speed.\n4. Implementation: build a working baseline, then improve with focused iterations.\n5. Validation: test conflict/error scenarios, not only happy-path behavior.\n6. Improvement roadmap: identify bottlenecks and optimize in next versions.\n\nIf you name a project (CliniQ, SafeRoute, Academic Setu, Fake News, AI Chatbot, Event Image Finder, VoiceGuru, PromptGate), I can provide a deep project-specific complexity breakdown.`;
  }
